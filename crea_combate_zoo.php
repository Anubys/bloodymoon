...
echo "Te lanzas con los colmillos a su yugular y le haces $ataque1 puntos de daño";
 $consulta2="SELECT * FROM actores where nombre='lobo'";
    $consultavida = mysql_query($consulta2);
       while($row = mysql_fetch_assoc($consultavida)){
$muestravida=$row['vida'];
}
  $daño=$muestravida-$ataque1;
$upda=mysql_query("UPDATE `actores` SET `vida`='$daño' where`nombre`='lobo'")or die(mysql_error());
...
$consulta3="SELECT * FROM actores where nombre='lobo'";
...
echo "Esta muerta";
 echo "<script> document.location.href='muerte_contrincante_zoo.php';</script>";
	...
	$consulta4="SELECT * FROM actores where nombre='lobo'";
